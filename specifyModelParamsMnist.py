"""
[modelParams] = specifyModelParamsMnist(modifications) generates the param structure for the moth, ie
...
Output:
  1. modelParams = an instance of the class ModelParams, ready to pass to 'initializeConnectionMatrices'
"""
"""
...
def specifyModelParamsMnist(nF, goal):
    ...
    'mult' is different for P, L, and R. That is, a signal entering the
    ...
    KperPIfrMu = 1 / nK  # KperPfrMu; the ave fraction of KCs a PI connects to, The actual # will vary as a binomial distribution
    ...
    #      construction in 'initializeConnectionMatrices.py'
    #
    ...
    class ModelParams:
        def __init__(self):
            ...
            self.slopeParam = slopeParam
            self.symmetricAboutZero = symmetricAboutZero

            self.spontRmu = spontRmu
            self.spontRstd = spontRstd
            self.spontRbase = spontRbase
            self.spontRdistFlag = spontRdistFlag

            self.RperFFrMu = RperFFrMu
            self.RperFRawNum = RperFRawNum
            self.F2Rmu = F2Rmu
            self.F2Rstd = F2Rstd

            self.R2Gmu = R2Gmu
            self.R2Gstd = R2Gstd
            self.R2Pmult = R2Pmult
            self.R2Pstd = R2Pstd
            self.R2PImult = R2PImult
            self.R2PIstd = R2PIstd
            self.R2Lmult = R2Lmult
            self.R2Lstd = R2Lstd
            ...
            self.L2Rmult = L2Rmult
            self.L2Rstd = L2Rstd

            self.L2Pmult = L2Pmult
            self.L2Pstd = L2Pstd
            self.L2PImult = L2PImult
            self.L2PIstd = L2PIstd
            self.L2Lmult = L2Lmult
            self.L2Lstd = L2Lstd

            self.GsensMu = GsensMu
            self.GsensStd = GsensStd

            self.PIfr = PIfr
            self.G2PImu = G2PImu
            self.G2PIstd = G2PIstd

            self.KperPfrMu = KperPfrMu
            self.KperPIfrMu = KperPIfrMu
            self.GperPIfrMu = GperPIfrMu
            self.P2Kmu = P2Kmu
            self.P2Kstd = P2Kstd
            self.PI2Kmu = PI2Kmu
            self.PI2Kstd = PI2Kstd

            self.kGlobalDampFactor = kGlobalDampFactor
            self.kGlobalDampStd = kGlobalDampStd

            self.KperEfrMu = KperEfrMu
            self.K2Emu = K2Emu
            self.K2Estd = K2Estd

            self.octoNegDiscount = octoNegDiscount
            self.octostdFactor = octoStdFactor
            self.octo2Gmu = octo2Gmu
            self.octo2Gstd = octo2Gstd
            self.octo2Pmult = octo2Pmult
            self.octo2Pstd = octo2Pstd
            self.octo2PImult = octo2PImult
            self.octo2PIstd = octo2PIstd
            self.octo2Lmult = octo2Lmult
            self.octo2Lstd = octo2Lstd
            self.octo2Rmult = octo2Rmult
            self.octo2Rstd = octo2Rstd
            self.octo2Kmu = octo2Kmu
            self.octo2Kstd = octo2Kstd
            self.octo2Emu = octo2Emu
            self.octo2Estd = octo2Estd

            self.hebTauPK = hebTauPK
            self.hebTauPIK = hebTauPIK
            self.hebTauKE = hebTauKE
            self.hebMaxPK = hebMaxPK
            self.hebMaxPIK = hebMaxPIK
            self.hebMaxKE = hebMaxKE

            self.sparsityTarget = sparsityTarget
            self.octoSparsityTarget = octoSparsityTarget

            self.dieBackTauPK = dieBackTauPK
            self.dieBackTauPIK = dieBackTauPIK
            self.dieBackTauKE = dieBackTauKE

            self.noiseR = noiseR
            self.RnoiseStd = RnoiseStd
            self.noiseP = noiseP
            self.PnoiseStd = PnoiseStd
            self.noisePI = noisePI
            self.PInoiseStd = PInoiseStd
            self.noiseL = noiseL
            self.LnoiseStd = LnoiseStd
            self.noiseK = noiseK
            self.KnoiseStd = KnoiseStd
            self.noiseE = noiseE
            self.EnoiseStd = EnoiseStd
            ...
            self.PI2K = None
            self.K2E = None
            self.Rspont = None  # col vector

            self.noiseRvec = None
            self.noisePvec = None
            self.noisePIvec = None
            self.noiseLvec = None
            self.noiseKvec = None
            self.noiseEvec = None
            self.kGlobalDampVec = None
